...
//     extensions).
//
// To tests changes to this file:
//
// ```
// module load cce/8.6.5 # Pick the version you want to tests.
// cd boost/libs/config/tests/all
...
//
// Note: If a tests '*_fail.cpp' file compiles, but fails to run, then it is
// reported as a defect. However, this is not actually a defect. This is an
// area where the tests system is somewhat broken. Tests that are failing
...
// you should pay attention to (in addition to macros that cause tests
// failures).
...
// way the equality tests can pass is if _RELEASE_PATCHLEVEL expands to x.
//
...
#define BOOST_HAS_FLOAT128
#define BOOST_HAS_PTHREAD_YIELD // This is a platform macro, but it improves tests results.
...
#undef  BOOST_NO_CXX11_FUNCTION_TEMPLATE_DEFAULT_ARGS
#define BOOST_NO_CXX11_SFINAE_EXPR // This is correct, even though '*_fail.cpp' tests fails.
#undef  BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX
#undef  BOOST_NO_CXX11_VARIADIC_MACROS
#undef  BOOST_NO_CXX11_VARIADIC_TEMPLATES
// 'BOOST_NO_DEDUCED_TYPENAME' tests is broken. The tests files are enabled /
...
//
// The 'no_ded_typename_pass.cpp' tests should always compile and run
...
// Therefore, if you wish to tests changes to 'BOOST_NO_DEDUCED_TYPENAME',
