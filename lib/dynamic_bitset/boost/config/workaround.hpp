...
//   (symbol tests) + 1        =>   if (symbol tests) then 2 else 1
//   1 % ((symbol tests) + 1)  =>   if (symbol tests) then 1 else 0
//
// The complication with % is for cooperation with BOOST_TESTED_AT().
// When "tests" is BOOST_TESTED_AT(x) and
// BOOST_DETECT_OUTDATED_WORKAROUNDS is #defined,
//
//   symbol tests              =>   if (symbol <= x) then 1 else -1
//   (symbol tests) + 1        =>   if (symbol <= x) then 2 else 0
//   1 % ((symbol tests) + 1)  =>   if (symbol <= x) then 1 else divide-by-zero
//
...
#define BOOST_WORKAROUND(symbol, tests) 0
