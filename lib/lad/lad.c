...
c_bool checkLAD(c_bool induced, int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	// return c_true if G_(u,v) has a adj(u)-covering matching; c_false otherwise
	int u2, v2, i, j;
	int nbMatched = 0;
	...
	if (Gp->nbAdj[u]==1){
		...
		if ((v2 != -1) && (isInD(u2,v2,D))) return c_true;
	...
	if (nbMatched == Gp->nbAdj[u]) return c_true; // The matching still covers adj(u)
	...
	for (i=0; i<Gp->nbAdj[u]; i++){
		...
		if (nbComp[i]==0) return c_false; // u2 has no compatible vertex in adj[v]
	...
	if (!updateMatching(Gp->nbAdj[u],nbNum,nbComp,firstComp,comp,matchedWithU)) return c_false;
