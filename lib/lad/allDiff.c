...
c_bool updateMatching(int sizeOfU, int sizeOfV, int* degree, int* firstAdj, int*  adj, int* matchedWithU){
	// input:
	...
	// returns c_true if there exists a matching that covers U, i.e., if for every u in 0..nbU-1, 
	// there exists a different v in 0..nb-1 such that v is adjacent to u;
	// returns c_false otherwise
	
	if (sizeOfU>sizeOfV) return c_false; // trivial case of infeasibility
...
void DFS(int nbU, int nbV, int u, c_bool* marked, int* nbSucc, int** succ, int* matchedWithU, int* order, int* nb){
...
void SCC(int nbU, int nbV, int* numV, int* numU, 
	...
	memset(marked,c_false,nbU*sizeof(int));
	nb=nbU-1;
	for (u=0; u<nbU; u++)
...
c_bool ensureGACallDiff(c_bool induced, Tgraph* Gp, Tgraph* Gt, Tdomain* D){
	...
	//                return c_false if an inconsistency is detected; c_true otherwise
	...
	c_bool used[Gp->nbVertices][Gt->nbVertices];
	memset(numV,c_false,Gt->nbVertices*sizeof(int));
	memset(nbSucc,0,Gt->nbVertices*sizeof(int));
	memset(nbPred,0,Gp->nbVertices*sizeof(int));
	memset(numU,c_false,Gp->nbVertices*sizeof(int));
	...
	for (u=0; u<Gp->nbVertices; u++){
		...
		for (i=0; i<D->nbVal[u]; i++){
			...
			if ((!used[u][v]) && (numV[v]!=numU[u]) && (D->globalMatchingP[u]!=v))
				if (!removeValue(u,v,D,Gp,Gt)) return c_false;
		}
